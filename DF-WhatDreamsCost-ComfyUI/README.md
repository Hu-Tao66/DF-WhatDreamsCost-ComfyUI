...
This project is based on [yg496/CS-WhatDreamsCost-ComfyUI](https://github.com/yg496/CS-WhatDreamsCost-ComfyUI), which itself is based on [WhatDreamsCost/WhatDreamsCost-ComfyUI](https://github.com/WhatDreamsCost/WhatDreamsCost-ComfyUI).
The upstream authors are credited here as the pre-existing source. This repository is a DF fork that adds a separate `DF-` namespace, so it can be installed beside the original and CS plugins without replacing their node IDs.

## Overview

`DF-WhatDreamsCost-ComfyUI` focuses on an automated LTX storyboard workflow:
...
- Keep the original LTX Director timeline workflow, while registering all public nodes under `DF-...` IDs.
- Add guide latent size alignment in `DF-LTXDirectorGuide` to reduce LTX guide insertion size errors.

这个分支的核心目标是把“六宫格图像 -> 六段分镜 -> 可编辑导演台时间线 -> LTX 生成”尽量自动化，同时保留前端手动修改能力。

## DF Namespace

This fork intentionally registers public nodes with `DF-` IDs. The legacy WhatDreamsCost node IDs are not registered here, so shared platforms such as RunningHub should not treat this package as a replacement for the original plugin.

| Original-style node | DF fork node ID |
| --- | --- |
| `LTXDirector` | `DF-LTXDirector` |
| `LTXAutoDirector` | `DF-LTXAutoDirector` |
| `LTXSixGridDirector` | `DF-LTXSixGridDirector` |
| `LTXDirectorGuide` | `DF-LTXDirectorGuide` |
| `LTXKeyframer` | `DF-LTXKeyframer` |
| `LTXSequencer` | `DF-LTXSequencer` |
| `MultiImageLoader` | `DF-MultiImageLoader` |
| `SpeechLengthCalculator` | `DF-SpeechLengthCalculator` |
| `LoadAudioUI` | `DF-LoadAudioUI` |
| `LoadVideoUI` | `DF-LoadVideoUI` |

Old workflows that reference unprefixed WhatDreamsCost node IDs need to be updated to the matching `DF-...` node IDs before they can use this fork.
...
3. Restart ComfyUI.
4. Search for `DF-` in the ComfyUI node menu.
