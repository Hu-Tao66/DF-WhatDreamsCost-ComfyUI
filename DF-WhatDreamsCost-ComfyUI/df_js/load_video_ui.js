...
app.registerExtension({
    name: "Comfy.DF-LoadVideoUI",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "DF-LoadVideoUI") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                node.updatePreview = function (filename) {
                    ...
                    if (filename.match(/^[a-zA-Z]:\\/) || filename.startsWith('/')) {
                        url = api.apiURL(`/df_video_ui_custom_view?filename=${encodeURIComponent(filename)}`);
                    } else {
                    ...
                    try {
                        ...
                        if (file.size > CHUNK_SIZE) {
                            ...
                            for (let i = 0; i < totalChunks; i++) {
                                ...
                                const resp = await api.fetchApi("/df_video_ui_upload_chunk", {
                                    method: "POST",
                                    body: formData,
                                });
