...
// Global registry to track all DF-LTXKeyframer nodes across all subgraphs
window._DFLTXKeyframerGlobalNodes = window._DFLTXKeyframerGlobalNodes || new Set();
...
// Finds all other DF-LTXKeyframer nodes globally and mirrors the value to them
function syncWidgetAcrossNodes(sourceNode, widgetName, value) {
    if (!window._DFLTXKeyframerGlobalNodes) return;
    
    for (const targetNode of window._DFLTXKeyframerGlobalNodes) {
        // Target all OTHER DF-LTXKeyframer nodes by direct object reference
...
app.registerExtension({
    name: "Comfy.DF-LTXKeyframer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "DF-LTXKeyframer") return;

        // Register this node instance globally
        window._DFLTXKeyframerGlobalNodes.add(node);
        ...
        // Helper: read image count from a connected DF-MultiImageLoader node
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "DF-MultiImageLoader") {
                    return originNode;
                }
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode && innerNode.comfyClass === "DF-MultiImageLoader") {
                            return innerNode;
                        }
                    } catch (e) {
            ...
            // If there is EXACTLY ONE DF-MultiImageLoader in the workspace, safely assume that's the one.
            let multiImageLoaders = [];
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "DF-MultiImageLoader") {
        ...
        node.onRemoved = function() {
            ...
            window._DFLTXKeyframerGlobalNodes.delete(node);
