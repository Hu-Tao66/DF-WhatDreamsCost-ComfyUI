...
const SIX_GRID_INPUT_LABELS = {
  ...
  custom_height: "\u8f93\u51fa\u9ad8\u5ea6",
  grid_layout: "\u516d\u5bab\u683c\u5e03\u5c40",
  auto_crop_borders: "\u81ea\u52a8\u88c1\u6389\u516d\u5bab\u683c\u8fb9\u6846",
  border_sensitivity: "\u8fb9\u6846\u68c0\u6d4b\u7075\u654f\u5ea6",
  border_crop_px: "\u5206\u9694\u7ebf\u5b89\u5168\u88c1\u526a\u50cf\u7d20",
...
if (!document.getElementById("df-whatdreamscost-styles")) {
  const styleEl = document.createElement("style");
  styleEl.id = "df-whatdreamscost-styles";
...
function prBoolWidgetValue(node, name, fallback = false) {
  const value = prGetWidgetValue(node, name, fallback);
  if (typeof value === "boolean") return value;
  if (typeof value === "number") return value !== 0;
  const text = String(value ?? "").trim().toLowerCase();
  if (["true", "1", "yes", "on", "\u662f", "\u5f00", "\u542f\u7528"].includes(text)) return true;
  if (["false", "0", "no", "off", "\u5426", "\u5173", "\u7981\u7528"].includes(text)) return false;
  return fallback;
}

function prNumberWidgetValue(node, name, fallback = 0) {
  const value = Number(prGetWidgetValue(node, name, fallback));
...
function prProjectionBandScore(projection, expected) {
  const len = projection.length;
  if (!len) return 0;
  const radius = Math.max(2, Math.round(len / 64));
  const sampleRadius = Math.max(radius * 3, 6);
  const avg = (start, end) => {
    start = Math.max(0, Math.round(start));
    end = Math.min(len, Math.round(end));
    if (end <= start) return projection[Math.max(0, Math.min(len - 1, Math.round(expected)))] || 0;
    let total = 0;
    for (let i = start; i < end; i++) total += projection[i] || 0;
    return total / (end - start);
  };

  let best = 0;
  const start = Math.max(0, Math.round(expected) - sampleRadius);
  const end = Math.min(len - 1, Math.round(expected) + sampleRadius);
  for (let i = start; i <= end; i++) {
    const band = avg(i - radius, i + radius + 1);
    const left = avg(i - sampleRadius * 2, i - sampleRadius);
    const right = avg(i + sampleRadius, i + sampleRadius * 2);
    const local = (left + right) / 2;
    const brightLine = Math.max(0, band - 0.82) * 2.0;
    const darkLine = Math.max(0, 0.06 - band) * 1.2;
    const contrast = Math.abs(band - local);
    const distance = Math.abs(i - expected) / Math.max(1, sampleRadius);
    best = Math.max(best, brightLine + darkLine + contrast - distance * 0.08);
  }
  return best;
}

function prMeasureImageProjections(img, maxDim = 700) {
  ...
  try {
    ...
    const xSaturationProjection = new Array(width).fill(0);
    const ySaturationProjection = new Array(height).fill(0);

    for (let y = 0; y < height; y++) {
      let rowTotal = 0;
      let rowSaturationTotal = 0;
      for (let x = 0; x < width; x++) {
        ...
        xProjection[x] += lum;
        xSaturationProjection[x] += saturation;
        rowTotal += lum;
        rowSaturationTotal += saturation;
      }
      yProjection[y] = rowTotal / width;
      ySaturationProjection[y] = rowSaturationTotal / width;
    }
    for (let x = 0; x < width; x++) {
      xProjection[x] /= height;
      xSaturationProjection[x] /= height;
    }
    return { width, height, scale, data, xProjection, yProjection, xSaturationProjection, ySaturationProjection };
  } catch (err) {
    console.warn("[DF WhatDreamsCost] Could not read six-grid image pixels:", err);
    return null;
  }
}
...
function prDetectGridLayoutFromImage(img) {
  ...
  const { width, height, xProjection, yProjection } = measured;
    ...
    const score2x3 =
      prProjectionBandScore(xProjection, width / 2) +
      prProjectionBandScore(yProjection, height / 3) +
      prProjectionBandScore(yProjection, height * 2 / 3);
    const score3x2 =
      prProjectionBandScore(xProjection, width / 3) +
      prProjectionBandScore(xProjection, width * 2 / 3) +
      prProjectionBandScore(yProjection, height / 2);
...
function prResolveGridDims(node, source = null, img = null) {
  const setting = source?.gridLayout || prGetGridLayoutSetting(node);
  const explicit = prGridDimsForLayout(setting);
  if (explicit) return { ...explicit, gridLayout: setting };

  const detectedLayout = img ? prDetectGridLayoutFromImage(img) : "auto";
  const detected = prGridDimsForLayout(detectedLayout) || { cols: source?.cols || 3, rows: source?.rows || 2 };
  return { ...detected, gridLayout: "auto", detectedLayout };
}

function prProjectionMask(projection, saturationProjection, sensitivity) {
  const sens = clamp(Number(sensitivity) || 0.10, 0.01, 0.45);
  const brightThreshold = Math.max(0.60, 1 - Math.max(sens, 0.16) * 2.2);
  const saturationThreshold = Math.min(0.40, 0.28 + sens * 0.4);
  return projection.map((value, index) => {
    const saturation = Array.isArray(saturationProjection) ? (saturationProjection[index] ?? 0) : 0;
    return (value >= brightThreshold || value <= sens) && saturation <= saturationThreshold;
  });
}

function prSafeBorderCropPx(cellSize, cropPx, scale = 1) {
  cellSize = Math.max(1, Number(cellSize) || 1);
  scale = Math.max(0.001, Number(scale) || 1);
  const requested = Math.max(0, Math.round((Number(cropPx) || 0) * scale));
  const automatic = Math.max(Math.round(8 * scale), Math.round(cellSize * 0.022), 1);
  const maxSafe = Math.max(0, Math.min(Math.round(48 * scale), Math.floor(cellSize / 5)));
  return Math.min(Math.max(requested, automatic), maxSafe);
}

function prMaskRuns(mask, start, end) {
  const runs = [];
  let index = Math.max(0, Math.round(start));
  end = Math.min(mask.length, Math.round(end));
  while (index < end) {
    while (index < end && !mask[index]) index++;
    const runStart = index;
    while (index < end && mask[index]) index++;
    if (index > runStart) runs.push([runStart, index]);
  }
  return runs;
}

function prFindBorderBand(mask, expected, axisLen, cellSize) {
  expected = clamp(Math.round(expected), 0, axisLen);
  const searchRadius = Math.max(2, Math.min(64, Math.round(cellSize / 8)));
  const maxBandWidth = Math.max(2, Math.min(64, Math.round(cellSize / 6)));

  if (expected <= 0) {
    const runs = prMaskRuns(mask, 0, Math.min(axisLen, searchRadius + 1));
    if (runs.length && runs[0][0] <= 2) return [0, Math.min(runs[0][1], maxBandWidth)];
    return [0, 0];
  }

  if (expected >= axisLen) {
    const runs = prMaskRuns(mask, Math.max(0, axisLen - searchRadius - 1), axisLen);
    if (runs.length && axisLen - runs[runs.length - 1][1] <= 2) {
      const run = runs[runs.length - 1];
      return [Math.max(axisLen - maxBandWidth, run[0]), axisLen];
    }
    return [axisLen, axisLen];
  }

  const runs = prMaskRuns(mask, expected - searchRadius, expected + searchRadius + 1)
    .filter((run) => run[1] - run[0] <= maxBandWidth);
  if (!runs.length) return [expected, expected];

  const distance = (run) => {
    if (run[0] <= expected && expected < run[1]) return 0;
    return Math.min(Math.abs(expected - run[0]), Math.abs(expected - (run[1] - 1)));
  };
  const best = runs.reduce((winner, run) => distance(run) < distance(winner) ? run : winner, runs[0]);
  return distance(best) <= searchRadius ? best : [expected, expected];
}

function prBuildAxisIntervals(projection, saturationProjection, parts, sensitivity, cropPx, scale) {
  const axisLen = projection.length;
  parts = Math.max(1, Math.round(parts || 1));
  if (parts === 1) return [[0, Math.round(axisLen / scale)]];

  const mask = prProjectionMask(projection, saturationProjection, sensitivity);
  const cellSize = Math.max(1, axisLen / parts);
  const bands = [];
  for (let i = 0; i <= parts; i++) {
    bands.push(prFindBorderBand(mask, Math.round(i * axisLen / parts), axisLen, cellSize));
  }

  const intervals = [];
  const safeCrop = prSafeBorderCropPx(cellSize, cropPx, scale);
  for (let i = 0; i < parts; i++) {
    ...
    start = clamp(start, 0, axisLen - 1);
    end = clamp(end, start + 1, axisLen);
...
function prPixelBorderLike(data, width, x, y, sensitivity) {
  x = clamp(Math.round(x), 0, width - 1);
  const offset = (Math.round(y) * width + x) * 4;
  const r = data[offset] / 255;
  const g = data[offset + 1] / 255;
  const b = data[offset + 2] / 255;
...
function prEdgeBorderRatio(measured, rect, edge, offset, sensitivity) {
  const { data, width, height } = measured;
  const x0 = clamp(Math.round(rect.x0), 0, width - 1);
  const x1 = clamp(Math.round(rect.x1), x0 + 1, width);
  const y0 = clamp(Math.round(rect.y0), 0, height - 1);
  const y1 = clamp(Math.round(rect.y1), y0 + 1, height);
  let hits = 0;
  let total = 0;

  if (edge === "left" || edge === "right") {
    const x = edge === "left" ? x0 + offset : x1 - offset - 1;
    for (let y = y0; y < y1; y++) {
      if (prPixelBorderLike(data, width, x, y, sensitivity)) hits++;
      total++;
    }
  } else {
    const y = edge === "top" ? y0 + offset : y1 - offset - 1;
    for (let x = x0; x < x1; x++) {
      if (prPixelBorderLike(data, width, x, y, sensitivity)) hits++;
      total++;
    }
...
function prTrimRectBorderEdges(measured, rect, settings) {
  if (!measured?.data) return rect;

  const minDim = Math.min(rect.x1 - rect.x0, rect.y1 - rect.y0);
  const safeCrop = prSafeBorderCropPx(minDim, settings.borderCropPx || 0, measured.scale);
  let maxTrim = Math.max(8, Math.round(minDim * 0.05), safeCrop + 8);
  maxTrim = Math.min(maxTrim, Math.max(0, Math.floor(minDim / 6)), 48);
  if (maxTrim <= 0) return rect;

  const threshold = 0.42;
  let left = 0;
  while (left < maxTrim && rect.x0 + left < rect.x1 - 2) {
    if (prEdgeBorderRatio(measured, rect, "left", left, settings.borderSensitivity) < threshold) break;
    left++;
  }

  let right = 0;
  while (right < maxTrim && rect.x1 - right > rect.x0 + left + 2) {
    if (prEdgeBorderRatio(measured, rect, "right", right, settings.borderSensitivity) < threshold) break;
    right++;
  }

  let top = 0;
  while (top < maxTrim && rect.y0 + top < rect.y1 - 2) {
    if (prEdgeBorderRatio(measured, rect, "top", top, settings.borderSensitivity) < threshold) break;
    top++;
  }

  let bottom = 0;
  while (bottom < maxTrim && rect.y1 - bottom > rect.y0 + top + 2) {
    if (prEdgeBorderRatio(measured, rect, "bottom", bottom, settings.borderSensitivity) < threshold) break;
    bottom++;
  }
  ...
  if (trimmed.x1 - trimmed.x0 < 8 || trimmed.y1 - trimmed.y0 < 8) return rect;
  return trimmed;
}
...
function prBuildSixGridCrop(source, img, node) {
  ...
  const settings = {
    ...
    borderCropPx: source?.borderCropPx ?? prGetBorderCropSettings(node).borderCropPx,
  };

  if (!settings.autoCropBorders || !img?.naturalWidth || !img?.naturalHeight) {
    const xIntervals = Array.from({ length: cols }, (_, i) => [
      Math.round(i * img.naturalWidth / cols),
      Math.round((i + 1) * img.naturalWidth / cols),
    ]);
    const yIntervals = Array.from({ length: rows }, (_, i) => [
      Math.round(i * img.naturalHeight / rows),
      Math.round((i + 1) * img.naturalHeight / rows),
    ]);
    return {
      xIntervals,
      yIntervals,
  ...
  const measured = prMeasureImageProjections(img, 900);
  if (!measured) return null;

  const xIntervals = prBuildAxisIntervals(
    measured.xProjection,
    measured.xSaturationProjection,
    cols,
    settings.borderSensitivity,
    settings.borderCropPx,
    measured.scale
  );
  const yIntervals = prBuildAxisIntervals(
    measured.yProjection,
    measured.ySaturationProjection,
    rows,
    settings.borderSensitivity,
    settings.borderCropPx,
    measured.scale
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      rects.push({
        ...
        y0: Math.round(trimmed.y0 / measured.scale),
        y1: Math.round(trimmed.y1 / measured.scale),
      });
    }
  }

  return { xIntervals, yIntervals, rects };
}
...
class TimelineEditor {
  ...
  createDOM() {
    ...
    helpBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      window.open("https://github.com/your-name/DF-WhatDreamsCost-ComfyUI", "_blank");
    });
  ...
  drawSixGridStoryboardSegment(seg, startX, pxWidth) {
    ...
    const rect = source.cropGrid?.rects?.[idx];
    const xInterval = source.cropGrid?.xIntervals?.[col];
    const yInterval = source.cropGrid?.yIntervals?.[row];
    const sx = rect ? rect.x0 : (xInterval ? xInterval[0] : Math.round(col * img.naturalWidth / source.cols));
    const sy = rect ? rect.y0 : (yInterval ? yInterval[0] : Math.round(row * img.naturalHeight / source.rows));
    const sx2 = rect ? rect.x1 : (xInterval ? xInterval[1] : Math.round((col + 1) * img.naturalWidth / source.cols));
    const sy2 = rect ? rect.y1 : (yInterval ? yInterval[1] : Math.round((row + 1) * img.naturalHeight / source.rows));
  ...
  showSettingsMenu(anchorEl) {
    ...
    // --- Six-grid Layout ---
    const gridLayoutWidget = this.node.widgets?.find(w => w.name === "grid_layout");
    if (gridLayoutWidget) {
      const select = document.createElement("select");
      select.className = "pr-settings-select";
      const labels = Object.values(SIX_GRID_COMBO_VALUE_LABELS.grid_layout);
      for (const label of labels) {
        const option = document.createElement("option");
        option.value = label;
        option.textContent = label;
      ...
      const current = prNormalizeGridLayout(gridLayoutWidget.value);
    ...
    // --- Six-grid Border Crop ---
    const autoCropWidget = this.node.widgets?.find(w => w.name === "auto_crop_borders");
    if (autoCropWidget) {
      const cb = document.createElement("input");
      cb.type = "checkbox";
      cb.checked = !!autoCropWidget.value;
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (LTX_DIRECTOR_NODE_TYPES.includes(nodeData.name) || SIX_GRID_NODE_TYPES.includes(nodeData.name)) {
