...
// Global registry to track all DF-LTXSequencer nodes across all subgraphs
window._DFLTXSequencerGlobalNodes = window._DFLTXSequencerGlobalNodes || new Set();
...
function syncFullStateAcrossNodes(sourceNode) {
    if (!window._DFLTXSequencerGlobalNodes) return;
    
    for (const targetNode of window._DFLTXSequencerGlobalNodes) {
...
app.registerExtension({
    name: "Comfy.DF-LTXSequencer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "DF-LTXSequencer") return;

        // Register this node instance globally
        window._DFLTXSequencerGlobalNodes.add(node);
        ...
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "DF-MultiImageLoader") return originNode;
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode?.comfyClass === "DF-MultiImageLoader") return innerNode;
                    } catch (e) {}
                }
                return null;
            }
            ...
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "DF-MultiImageLoader") multiImageLoaders.push(n);
        ...
        node.onRemoved = function() {
            window._DFLTXSequencerGlobalNodes.delete(node);
            clearInterval(pollInterval);
